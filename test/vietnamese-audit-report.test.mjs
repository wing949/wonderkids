...
test('báo cáo ghi đủ danh mục SGK, không nhầm bài chờ duyệt thành Luyện thêm', async () => {
  ...
  try {
    ...
    assert.match(markdown, /Bài SGK đã xác minh\s*\|\s*14/);
    assert.match(markdown, /Hoạt động SGK đã xác minh\s*\|\s*119/);
    assert.match(markdown, /Danh mục SGK chờ đối chiếu nguyên văn\s*\|\s*362/);
    assert.match(markdown, /Bài đang khóa văn bản\/audio chính\s*\|\s*362/);
    assert.match(markdown, /Luyện thêm trong bài\s*\|\s*0/);
    assert.match(markdown, /Chưa phát hành nguyên văn SGK chưa duyệt/i);
    assert.match(
      markdown,
      /Transcript SGK có audio khớp transcript\s*\|\s*14\/14/,
      'Mỗi transcript đã duyệt phải có đúng cặp audio mới khớp hash và trang nguồn',
    );
    assert.match(
      markdown,
      /Cặp audio chính\/fallback đạt kiểm tra file\s*\|\s*14\/376/,
