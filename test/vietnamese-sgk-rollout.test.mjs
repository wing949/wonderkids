...
test('danh mục SGK tách rõ bài đã duyệt nguyên văn, bài chờ duyệt và Luyện thêm', () => {
  ...
  assert.equal(lessons.filter((lesson) => lesson.catalogSection === 'sgk').length, 16);
  assert.equal(lessons.filter((lesson) => lesson.catalogSection === 'sgk_pending').length, 360);
...
test('chỉ mở đọc mẫu khi transcript đã đối chiếu đúng trang SGK', () => {
  ...
  const unverifiedTranscript = curriculum.getLessonsForGradeAndSubject(2, 'vietnamese')
    .find((lesson) => lesson.id === 'tv-g2-b17');
