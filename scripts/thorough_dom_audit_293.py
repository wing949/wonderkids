...
# Precise OCR / Garbled / Question / Noise detector (avoids matching valid Vietnamese words)
OCR_GARBLED_REGEX = re.compile(
    r"[~¬|_=›‹«»\\\/^#$*@+§¿€]|"
    r"(\b(chữ cói|thế mờ|toán nứa|chững chọc|hỏn lên|rẻ lời|câu hỏi ø|3ð|3Ø|xi mm|ừựùừíiênHN|m g7|Z Mi|ty se|SÃI)\b)|"
...
# AI Paraphrase / Summary detector
...
for idx, lesson in enumerate(target_lessons, 1):
    ...
    text_to_audit = transcript_entries.get(lid, sample_text)
    ...
    # 2. Locked
    elif is_fallback or para_count == 0 or len(text_to_audit.strip()) == 0:
    ...
    # 3. AI Paraphrase / Summary
    elif AI_PARAPHRASE_REGEX.search(text_to_audit):
        match = AI_PARAPHRASE_REGEX.search(text_to_audit).group(0)
        status_label = "FAIL – AI TỰ BIÊN / DIỄN GIẢI LẠI (KHÔNG ĐÚNG NGUYÊN VĂN SGK)"
        cat = "fail_ai"
        reason = f"Văn bản cột phải do AI tự tóm tắt hoặc diễn giải lại (phát hiện đoạn tự chế: '{match}')."
        grade_stats[grade]["fail_ai"] += 1
        
    # 4. OCR noise or mixed exercises
    elif OCR_GARBLED_REGEX.search(text_to_audit):
        match = OCR_GARBLED_REGEX.search(text_to_audit).group(0)
        status_label = "FAIL – DÍNH RÁC OCR / LẪN BÀI TẬP TRANG SAU"
        cat = "fail_ocr"
        reason = f"Nội dung cột bên phải bị dính rác OCR hoặc lẫn câu hỏi bài tập trang sau (phát hiện rác: '{match}')."
        grade_stats[grade]["fail_ocr"] += 1
        
    # 5. Short text
    elif len(text_to_audit.strip()) < 30:
...
fails_ai = [it for it in all_audit_items if it["category"] == "fail_ai"]
fails_ocr = [it for it in all_audit_items if it["category"] == "fail_ocr" or it["category"] == "fail_short"]
lockeds = [it for it in all_audit_items if it["category"] == "locked"]
