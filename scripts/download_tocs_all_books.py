...
os.makedirs('tmp_pages/tocs', exist_ok=True)
...
for m in manifests:
    ...
    # In Vietnamese textbooks, TOC is at the front (pages 3, 4, 5, 6, 7, 8)
    for p_idx in [2, 3, 4, 5, 6, 7]:
        if p_idx < total:
            p = pages[p_idx]
            fname = p['imageUrl'].split('/')[-1]
            out_path = f"tmp_pages/tocs/g{g}_s{s}_p{p_idx+1}_{fname}"
            if not os.path.exists(out_path):
                try:
                    ...
                    print(f"Downloaded G{g} S{s} p{p_idx+1}: {fname}")
                except Exception as e:
                    print(f"Failed G{g} S{s} p{p_idx+1}: {e}")
