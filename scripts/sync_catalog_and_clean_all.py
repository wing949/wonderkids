...
for lesson in catalog:
    ...
    # Match gold standard
    gold = GOLD_STANDARDS.get(lesson_id) or GOLD_STANDARDS.get(stable_id) or GOLD_STANDARDS.get(explicit_id) or GOLD_STANDARDS.get(flat_id)
    ...
    # Store under all possible aliases
    ...
    if flat_id not in final_transcripts:
        final_transcripts[flat_id] = transcript_data
...
print(f"✅ Đã đồng bộ và làm sạch 100% cho {len(final_transcripts)} transcript keys trong SGK_VERIFIED_TRANSCRIPTS!")
